...
<jsp:useBean id="s_datas" scope="request" class="java.util.ArrayList" />
<jsp:useBean id="i_datas" scope="request" class="java.util.ArrayList" />
...
<body>
	...
	<%
		int length = s_datas.size() + 1;
		System.out.println(length);
		int[] arr = new int[length];
		double[] arr1 = new double[length];
		int i = 0;

		for (Patient_Sensor pan : (ArrayList<Patient_Sensor>) s_datas) {
	...
	<div align="center">
		...
		<div class="section-header">
			<h2 class="section-title wow fadeIn" data-wow-duration="1000ms" 
				data-wow-delay="0.3s" style="text-align:center;">실시간 모니터링</h2>
	...
	<div id='container'>
		...
		<h6 style="text-align:center;">측정 기록 테이블</h6>
		<HR>
		
		<table class="table-style-two" align="center">
			<tr>
				<th>온 도</th>
				<th>심 박</th>
			</tr>
			<%
				for (Patient_Sensor pa : (ArrayList<Patient_Sensor>) s_datas) {
			%>
			<tr>
				<td><%=pa.getTemp()%></td>
				<td><%=pa.getBpm()%></td>
	...
	<div id= 'box-center'>
		...
		<div id="curve_chart" style = "text-align:center;"></div>
	</div>
	
	
	<div id= 'box-right'>
			<%
				for (infusion_DO ab : (ArrayList<infusion_DO>) i_datas) {
			%>
		<h6 style="text-align:left;">수액 모니터링</h6>
		<HR>
		
		<table class="table-style-two">
			<tr>
				<th>식별 번호</th>	<td width = "90"><%=ab.getId()%></td>
			</tr>
			<tr>
				<th>이름</th> <td><%=ab.getInfusion_name()%></td>
			</tr>
			<tr>

				<th>스피드</th> <td><%=ab.getInfusion_total_amount()%>ml/s</td>
			</tr>

			<tr>
				<th>총량</th> <td><%=ab.getInfsuion_speed()%>ml</td>
			</tr>
			<tr>
				<th>남은시간</th><td><%=ab.getInfusion_reamin_time()%></td>
			</tr>
			<tr>
				<th>남은 양</th> <td><%=ab.getInfusion_remain_amount()%>ml</td>
			</tr>
			<tr>
				<th>병명</th><td><%=ab.getDisease()%></td>
			</tr>
